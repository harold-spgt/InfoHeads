...
public class CooldownGui extends AbstractGui {
    ...
    public CooldownGui(Player player, InfoHeads plugin, InfoHeadConfiguration infoHeadConfiguration) {
        ...
        getGui().setItem(5, 5, new GuiItem(ItemBuilder.from(Objects.requireNonNull(XMaterial.BARRIER.parseMaterial())).name(MessageUtil.getComponent(MessageUtil.Message.PREV_PAGE)).glow(true).build(), event -> new WizardGui(InfoHeads.getInstance(), getPlayer(), getConfiguration()).open()));
        getGui().setItem(5, 9, new GuiItem(ItemBuilder.from(Objects.requireNonNull(XMaterial.EMERALD_BLOCK.parseMaterial())).name(MessageUtil.getComponent(MessageUtil.Message.COMPLETE_ITEM_TITLE)).lore(MessageUtil.getComponentList(MessageUtil.Message.COMPLETE_ITEM_LORE)).glow(true).build(),
                event -> {
                    //Slots 3,4,5,6,7
